...
const TtsApiSection: React.FC<Props> = ({ baseUrl, apiKey, groupId, model, set }) => (
    <div className="bg-[#f3eef8]/60 backdrop-blur-sm p-5 rounded-3xl space-y-3 border border-[#e5ddf0]/40">
        ...
        <div><label className="text-[10px] font-bold text-[#b8aaa0] uppercase block mb-1">MiniMax API Key</label>
            <input type="password" value={apiKey} onChange={e => set('apiKey', e.target.value)} className="w-full bg-white/60 backdrop-blur-sm border border-[#e5ddf0]/50 rounded-xl px-3 py-2.5 text-sm font-mono focus:bg-white/80 transition-all" placeholder="Bearer Token" {...getGuardedInputProps({ kind: 'secret', field: 'tts-api-key' })} /></div>
