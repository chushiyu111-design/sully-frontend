                ...
                <section className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            {([
                                ...
                                return (
                                    <button
                                        key={opt.key}
                                        onClick={() => {
                                            const patch: any = { datePerspective: opt.key };
                                            if (opt.key === 'first' && (char.dateCharPerspective || 'third') === 'first') {
                                                patch.dateCharPerspective = 'third';
                                            }
                                            updateCharacter(char.id, patch);
                                        }}
                                    ...
                                    >
                                        ...
                                        <div className={`text-[10px] mt-0.5 transition-colors ${isActive ? 'text-blue-500 font-bold' : 'text-slate-400'}`}>{opt.desc}</div>
                                    </button>
                                );
                            })}
                        </div>
                    ...
                    <div className="mb-3">
                        ...
                        <div className="relative bg-slate-100 rounded-2xl p-1 flex gap-1">
                            <div
                                ...
                                style={{
                                    width: 'calc(50% - 4px)',
                                    left: (char.dateCharPerspective || 'third') === 'first' ? '4px' : 'calc(50% + 0px)'
                                }}
                            />
                            {([
                                { key: 'first' as const, label: '第一', desc: '我' },
                                { key: 'third' as const, label: '第三', desc: '他/名字' },
                            ]).map(opt => {
                                const isActive = (char.dateCharPerspective || 'third') === opt.key;
                                const isDisabled = opt.key === 'first' && (char.datePerspective || 'second') === 'first';
                                return (
                                    <button
                                        key={opt.key}
                                        disabled={isDisabled}
                                        onClick={() => updateCharacter(char.id, { dateCharPerspective: opt.key })}
                                        className={`relative z-10 flex-1 py-2.5 rounded-xl text-center transition-all duration-200 ${
                                            isDisabled ? 'opacity-30 cursor-not-allowed'
                                            : isActive ? 'text-slate-800'
                                            : 'text-slate-400 hover:text-slate-500 active:scale-95'
                                        }`}
                                    >
                                        <div className={`text-sm font-bold transition-colors ${isActive && !isDisabled ? 'text-slate-800' : ''}`}>{opt.label}</div>
                                        <div className={`text-[10px] mt-0.5 transition-colors ${isActive && !isDisabled ? 'text-rose-500 font-bold' : 'text-slate-400'}`}>
                                            {isDisabled ? '冲突' : opt.desc}
                                        </div>
                                    </button>
                                );
                            })}
                    ...
                    <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-xl p-3 border border-slate-100">
                        ...
                        <p className="text-[12px] text-slate-600 leading-relaxed italic">
                            {(() => {
                                const uPov = char.datePerspective || 'second';
                                const cPov = char.dateCharPerspective || 'third';
                                const userRef = uPov === 'first' ? '我' : uPov === 'third' ? userProfile.name : '你';
                                if (cPov === 'first') {
                                    return `[normal] 我的视线落在${userRef}身上，停了一拍才移开。`;
                                }
                                return `[normal] 他的视线落在${userRef}身上，停了一拍才移开。`;
                            })()}
                        </p>
                    </div>
                </section>
