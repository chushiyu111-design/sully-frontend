...
/**
 * 物理去重：检查新的 sourceMessageIds 是否与某条已有记忆的 sourceMessageIds 高度重合。
 * 如果重合率 >= 60%，说明是对同一段对话的重复提取，应该 update 而非 create。
 * 比 cosine 去重更精准——不受 LLM 措辞差异影响。
 */
function findSourceOverlap(
    newSourceIds: number[],
...
// allMemories is used for sourceMessageIds overlap dedup (Fix 2).
async function processResult(
    ...
    if (result.action === 'create') {
        ...
        // 第一层去重：sourceMessageIds 物理重合检测（不受 LLM 措辞差异影响）
